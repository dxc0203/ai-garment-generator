...
st.markdown(t("Here you can view, edit, and create new prompt templates for different tasks."))
...
def save_prompt_file(prompt_dir, filename, content):
    ...
    try:
        ...
        st.error(t("Failed to save file: {}").format(e))
        return False
...
for i, (tab_name, prompt_dir) in enumerate(PROMPT_TYPES.items()):
    with tab_list[i]:
        st.subheader(t(tab_name))
        
        prompt_files = get_prompt_files(prompt_dir)
        
        # --- Edit Existing Template Section ---
        st.header(t("Edit Existing Templates"))
        if not prompt_files:
            st.info(t("No prompt templates found for this type. Please create one below."))
        else:
            selected_template = st.selectbox(
                t("Select a template to view or edit"), 
            ...
            if selected_template:
                ...
                if st.button(t("Save Changes"), key=f"save_{prompt_dir}_{selected_template}"):
                    if save_prompt_file(prompt_dir, selected_template, edited_content):
                        st.success(t("Template '{}' saved successfully!").format(selected_template))
                        st.rerun()
        
        st.divider()

        # --- Create New Template Section ---
        st.header(t("Create New Template"))
        with st.form(key=f"new_template_form_{prompt_dir}"):
            new_filename = st.text_input(
                t("New Template Filename (must end with .txt)"),
            ...
            new_content = st.text_area(
                ...
                height=200,
                placeholder=t("Enter the full text of your new prompt here..."),
                key=f"content_input_{prompt_dir}"
            )
            
            submitted = st.form_submit_button(t("Create Template"))
            
            if submitted:
                if not new_filename.endswith(".txt"):
                    st.error(t("Filename must end with .txt"))
                elif not new_content:
                    st.error(t("Template content cannot be empty."))
                elif os.path.exists(os.path.join(prompt_dir, new_filename)):
                    st.error(t("A template with this filename already exists."))
                else:
                    if save_prompt_file(prompt_dir, new_filename, new_content):
                        st.success(t("New template '{}' created successfully!").format(new_filename))
